...
func executeCommands(ctx context.Context, device *DeviceInfo, failed *os.File) error {
   ...
   modes := ssh.TerminalModes{
      session.ReadTimeout: time.Duration(device.ReadTimeout) * time.Second,
   }
   ...
   for _, command := range device.Commands {
      ...
      // ssh.SendRequest, instead of ssh.Write, is used to execute commands
      if err := session.SendRequest("exec", []byte(command + "\n"), nil); err != nil {
         log.Printf("Error sending command to device %s: %v", device.IP, err)
         fmt.Fprintf(failed, "%s: Error sending command\n", device.IP)
         return err
      }

      // ssh.Read method is used to read output, instead of ssh.ReadUntil
      b, err := session.Read([]byte{'>'})
      if err != nil {
